...
QUALITY_CHECKLIST_REFERENCE = "references/quality_checklist.md"
...
def _requires_quality_checklist(content: str) -> bool:
    return QUALITY_CHECKLIST_REFERENCE in content
...
def validate_skill(skill_path):
    ...
    if _requires_quality_checklist(content):
        checklist_path = skill_path / "references" / "quality_checklist.md"
        if not checklist_path.exists():
            return False, (
                f"SKILL.md requires '{QUALITY_CHECKLIST_REFERENCE}', "
                "but the file is missing"
            )
        try:
            checklist_text = checklist_path.read_text(encoding="utf-8")
        except UnicodeDecodeError as exc:
            return False, f"{QUALITY_CHECKLIST_REFERENCE} must be valid UTF-8 text: {exc}"
        if not checklist_text.strip():
            return False, f"{QUALITY_CHECKLIST_REFERENCE} cannot be empty"
