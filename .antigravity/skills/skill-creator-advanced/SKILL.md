...
5) 建立後立即補完 `references/quality_checklist.md`，把格式確認、要求/規範確認、常見錯誤確認與最終 PASS/FAIL gate 當成單一檢核面板。
6) 先決定這個 skill 是 `router`、`executor`、`ops` 還是 `utility`，再補完 `SKILL.md` 的決策邊界、`output contract`、`default follow-through policy`、few-shot 與 metadata surface。
7) 若 skill 會包工具、MCP 或 function calling，先檢查 tool/function 名稱、參數描述、enum 與 active tool set 是否足夠清楚。
8) 做格式、相容性與引用檢查，並把結果回填到 `references/quality_checklist.md`：
...
12) 若要優化 description 的觸發品質，另外準備 trigger eval set，再跑：
...
- 每個 skill 都必須在 `references/quality_checklist.md` 保留一份 readiness checklist；至少涵蓋格式確認、要求/規範確認、常見錯誤確認與最終 PASS/FAIL。
...
- 把上述檢查結果回填到 `references/quality_checklist.md`；若 checklist 缺失、未更新或無法解釋目前狀態，視為尚未完成 readiness audit。
...
5) **每個 skill 都要有 `references/quality_checklist.md`**
- 這份 checklist 是 readiness gate，不是可有可無的附件。
- 至少要記錄格式確認、要求/規範確認、常見錯誤確認與最終 PASS/FAIL。
- 若檢查結果沒有回填到 checklist，視為尚未完成驗證。
...
- `references/quality_checklist.md`（含本輪檢查結論與證據）
